...
export const loadPublicProfile = cache(async (slug: string) => {
  ...
  const { data } = await createAdminClient().from("characters")
    .select("id, name, slug, account_type, combat_level, total_level, visibility, last_synced_at, show_recent_collections, collection_log_obtained_count, collection_log_total_count")
